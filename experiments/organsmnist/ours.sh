            ...
            for classifier in "${classifiers[@]}"; do
                for anonymizer in "${anonymizers[@]}"; do
                    for anonymizer_lr in "${anonymizer_lr_values[@]}"; do
                        for cvae_beta in "${cvae_betas[@]}"; do
                            for cvae_var in "${cvae_vars[@]}"; do
                                for n_fedavg_rounds in "${n_fedavg_rounds_values[@]}"; do
                                    for epsilon in "${epsilons[@]}"; do
                                        for n_neighbors in "${n_neighbors_values[@]}"; do
                                            ...
                                            # DP:
                                            base_chkpts_dir="chkpts/organsmnist/$backbone/s_frac=${s_frac}/${n_clients}_clients/${split_method}/alpha=${alpha}/gen_seed=${client_generation_seed}/val_frac=${val_frac}/${classifier}_${anonymizer}/${cvae_layers}_layer_CVAE/anonymizer_lr=${anonymizer_lr}/cvae_beta=${cvae_beta}/${n_fedavg_rounds}_fedavg_rounds/${dp}/mgn=${max_grad_norm}-eps=${epsilon}-delta=${delta}/${current_time}" 
                                            base_results_dir="results/organsmnist/$backbone/s_frac=${s_frac}/${n_clients}_clients/${split_method}/alpha=${alpha}/gen_seed=${client_generation_seed}/val_frac=${val_frac}/global_sampling=${global_sampling}/${classifier}_${anonymizer}/${cvae_layers}_layer_CVAE/knn_metric=${knn_metric}/${knn_weights}/n_neighbors=${n_neighbors}/classifier_optimizer=${classifier_optimizer}/${local_epochs}_local_epochs/anonymizer_lr=${anonymizer_lr}/cvae_beta=${cvae_beta}/cvae_var=${cvae_var}/${n_fedavg_rounds}_fedavg_rounds/${dp}/mgn=${max_grad_norm}-eps=${epsilon}-delta=${delta}/total_generated_factor=${total_generated_factor}/augmentation=${augmentation_strategy}/${current_time}"
                                            
                                            # Non-DP:
                                            # base_chkpts_dir="chkpts/organsmnist/$backbone/s_frac=${s_frac}/${n_clients}_clients/${split_method}/alpha=${alpha}/gen_seed=${client_generation_seed}/val_frac=${val_frac}/${classifier}_${anonymizer}/${cvae_layers}_layer_CVAE/anonymizer_lr=${anonymizer_lr}/cvae_beta=${cvae_beta}/${n_fedavg_rounds}_fedavg_rounds/${dp}/${current_time}" 
                                            # base_results_dir="results/organsmnist/$backbone/s_frac=${s_frac}/${n_clients}_clients/${split_method}/alpha=${alpha}/gen_seed=${client_generation_seed}/val_frac=${val_frac}/global_sampling=${global_sampling}/${classifier}_${anonymizer}/${cvae_layers}_layer_CVAE/knn_metric=${knn_metric}/${knn_weights}/n_neighbors=${n_neighbors}/classifier_optimizer=${classifier_optimizer}/${local_epochs}_local_epochs/anonymizer_lr=${anonymizer_lr}/cvae_beta=${cvae_beta}/cvae_var=${cvae_var}/${n_fedavg_rounds}_fedavg_rounds/${dp}/total_generated_factor=${total_generated_factor}/augmentation=${augmentation_strategy}/${current_time}"
                                            ...
                                            for SEED in "${SEEDS[@]}"; do
                                                ...
                                                echo "$BEST_ACC $BEST_WEIGHT $BEST_BALANCED_ACC $BEST_WEIGHT_BALANCED $BEST_AUC $BEST_WEIGHT_AUC $BEST_F1_MACRO $BEST_WEIGHT_F1_MACRO $BEST_F1_WEIGHTED $BEST_WEIGHT_F1_WEIGHTED $AVG_TEST_ACC $AVG_TEST_BALANCED_ACC $AVG_TEST_AUC $AVG_TEST_F1_MACRO $AVG_TEST_F1_WEIGHTED" >> "$results_all_seeds_file" 
